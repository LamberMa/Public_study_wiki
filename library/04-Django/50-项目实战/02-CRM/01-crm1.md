...
```python
admin.site.register---site----site=AdminSite()这是一个对象，这个在程序的一开始只会执行一次，也就是说这其实是一个单例模式---找到register方法：
 
"""
可以看到这里的register方法传递了self，model_or_iterable，admin_class=None，**options
可以看到注册的模型类，可以接一个model或者是一个可迭代的对象，admin_class默认为None
所以我们注册的时候不只是可以admin.site.register(models.AAA)这种写法，还可以
admin.site.register([models.AAA, models.BBB])这样写。

而且目前我们发现，这里还有一个admin_class，默认值为None，实际上写代码的时候也没单独传递过。
在下面的代码中：
if not admin_class:
        admin_class = ModelAdmin
如果没有设置的话，那么admin_class就是ModelAdmin，这个admin是from django.contrib.admin import ModelAdmin, actions在开头这里引入的。其实也就是我们平常注册的时候完整的内容其实是这样的。
admin.site.register(models.AAA, ModelAdmin)

如果传递进来的model是ModelBase的实例那么就会把model_or_iterable给转换成一个列表。
因为我们自己写的model类都是继承的models.Model，而Model本身又是继承自ModelBase的，因此这个判断是有效的
所以最终都是变成列表来进行操作的。
...
def register(self, model_or_iterable, admin_class=None, **options):
    ...
    for model in model_or_iterable:
        ...
        # 一开始的时候self._registry是一个空字典，如果model在里面的话就会抛出一个异常，告诉你已经注册过了，不要再重复注册了。
        ...
        if not model._meta.swapped:
            ...
            # 实例化admin class并保存到registry
            # 这里的admin_class当我们没填写的时候其实就是ModelAdmin，实例化的时候传递了俩参数
            # 其中一个就是我们定义的类名，再者就是self，这里的self是AdminSet的对象其实就是site！
            # 绕了一圈绕回来了。
            # 填充到registry这字典里。key就是我们写的类名，value就是ModelAdmin的对象
            self._registry[model] = admin_class(model, self)
```

最后实际的结果就是创造了一个字典，key是model类名，value是ModelAdmin的对象；这样达成了一个怎么样的效果呢？其实就是我们写的每一张表都放到这个字典，对应ModelAdmin的一个对象；
...
admin执行完了以后程序该运行起来了。执行manage.py，第一行读settings.py，然后执行admin.py。
