...
1. 服务器执行客户端发送save命令
   ...
   # 我们可以写一个定时的任务，定期的去把dump.rdb转移走。相当于一个备份，防止有人flushdb然后save。给自己留一个后路，同时保存固定的数量个，然后用对应的时间命名。
   ```
...
AOF持久化的操作原理是被当有修改数据库的命令被执行的时候，服务器就会将执行的命令写入到AOF文件的末尾。和RDB不一样，AOF保存的是执行的操作，而不是数据本身。因为AOF文件里储存了服务器执行过的所有数据库修改的命令，所以给定一个AOF文件，服务器只要重新执行一遍AOF文件里面包含的所有命令，就可以达到还原数据库的目的。
...
由于aof记录的是操作命令，所以说如果有人手贱执行了flushdb的话那么在aof的文件中也会有这个内容，紧急情况下，也可以手动编辑aof的备份文件，把最后执行的flushdb这条记录的内容手动删掉。

## 小结

![](http://omk1n04i8.bkt.clouddn.com/18-5-2/29823792.jpg)

一般来说为了数据的安全性，可以两种持久化都开启。然后还原数据的时候优先使用aof的数据进行还原。